...
/**
 * HACK: Resolves the internal Obsidian Editor prototype.
 * 
 * Obsidian does not expose the `ScrollableMarkdownEditor` class directly in its public API.
 * However, we need to extend it to create a fully functional editor that behaves exactly like
 * the main editor (with scrolling, folding, etc.).
 * 
 * This function works by:
 * 1. Creating a temporary "dummy" editor widget using the internal embed registry.
 * 2. Inspecting its prototype chain to find the constructor we need.
 * 3. Cleaning up the dummy widget.
 * 
 * @param app - The Obsidian App instance
 * @returns The constructor for the internal ScrollableMarkdownEditor class
 */
...
export class EmbeddableMarkdownEditor extends resolveEditorPrototype(app) implements ScrollableMarkdownEditor {
    ...
    constructor(app: App, container: HTMLElement, options: Partial<MarkdownEditorProps>) {
        ...
        // MOCKING THE VIEW STRUCTURE
        // The editor commands expect to run within a MarkdownView.
        // Since we are running in a standalone editor instance, we need to mock the structure
        // so that `this.owner` (which usually points to the View) has references to `editMode` and `editor`.
        // This allows standard Obsidian commands to work on this embedded editor.
        ...
        // FOCUS MANAGEMENT
        // We use `monkey-around` to patch `setActiveLeaf`.
        // If THIS editor has focus, we prevent Obsidian from stealing focus back to the main leaf.
        this.register(
